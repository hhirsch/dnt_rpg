...
_glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
  ...
  if (!file) {
    fprintf(stderr, "_glmWriteMTL() falhou: nao consegui abrir \"%s\".\n",
	    filename);
    exit(1);
  }
  free(filename);
  ...
  fprintf(file, "#  \n");
  fprintf(file, "#  GLM library by Nate Robins\n");
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
  ...
  printf("glmVertexNormals(): %d normais genradas\n", model->numnormals);
}
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
  ...
  if (mode & GLM_FLAT && !model->facetnorms) {
    printf("glmWriteOBJ() aviso: saida flat normal requerida "
	   "sem normais de faces definidas.\n");
    mode &= ~GLM_FLAT;
  }
  if (mode & GLM_SMOOTH && !model->normals) {
    printf("glmWriteOBJ() aviso: saida smooth normal requerida "
	   "sem normais definidas.\n");
    mode &= ~GLM_SMOOTH;
  }
  if (mode & GLM_TEXTURE && !model->texcoords) {
    printf("glmWriteOBJ() aviso: saida de coordenadas de textura resitada "
	   "sem coordenadas de textura definidas.\n");
    mode &= ~GLM_TEXTURE;
  }
  if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
    printf("glmWriteOBJ() aviso: saida flat normal requisitada "
	   "e saida smooth normal requisitada (usando entao smooth).\n");
    mode &= ~GLM_FLAT;
  }
  ...
  file = fopen(filename, "w");
  if (!file) {
    fprintf(stderr, "glmWriteOBJ() fallhou: nao pude abrir \"%s\" para escrever.\n",
	    filename);
    exit(1);
  }
